...
(function(window) {
    window.ig = {game: null,version: '1.15',global: window,modules: {},resources: [],ready: false,baked: false,nocache: '',ua: {},lib: 'lib/',_current: null,_loadQueue: [],_waitForOnload: 0,$: function(selector) {
...
ig.module('impact.sound').defines(function() {
    ig.SoundManager = ig.Class.extend({clips: {},volume: 1,channels: 8,format: 'mp3',init: function() {
...
ig.module('impact.map').defines(function() {
    ig.Map = ig.Class.extend({tilesize: 8,width: 1,height: 1,data: [[]],init: function(tilesize, data) {
...
ig.module('game.menus').requires('impact.font').defines(function() {
    ...
    PauseMenu = Menu.extend({name: 'Menu',clearColor: 'rgba(0,0,0,0.9)',itemClasses: [MenuItemSoundVolume, MenuItemMusicVolume, MenuItemResume]});
...
ig.module('game.words').defines(function() {
    WORDS = {2: ['на', 'не', 'но', 'об', 'за', 'ко', 'до', 'ре', 'ми', 'фа', 'ля', 'си', 'со', 'ёж', 'ад', 'по', 'ус', 'вы', 'бы', 'да', 'ил', 'юг', 'эй', 'яд', 'им', 'як', 'ну', 'ню'],3: ['без', 'про', 'ёрш', 'бой', 'бок', 'боб', 'ага', 'аид', 'год', 'гул', 'бог', 'век', 'вне', 'воз', 'еда', 'его', 'ещё', 'газ', 'гам', 'где', 'дан', 'дао', 'дар', 'два', 'дед', 'дно', 'для', 'ему', 'жар', 'жир', 'жук', 'зад', 'зал', 'зло', 'зря', 'зуб', 'ибо', 'ива', 'изо', 'или', 'икс', 'инь', 'ион', 'йог', 'йод', 'как', 'кий', 'код', 'ком', 'кто', 'куб', 'кум', 'лёд', 'лён', 'лад', 'лай', 'лак', 'лев', 'лик', 'лис', 'лих', 'лоб', 'лом', 'лук', 'луг', 'мёд', 'май', 'мак', 'мал', 'миг', 'мел'],4: ['ёжик', 'ёлка', 'авто', 'бита', 'близ', 'блик', 'блок', 'бокс', 'быть', 'ваза', 'вмиг', 'вода', 'глаз', 'глас', 'гиря', 'гель', 'гром', 'гуру', 'даже', 'дама', 'дуло', 'жнец', 'жила', 'жюри', 'зима', 'злак', 'зона', 'игра', 'идея', 'иная', 'ирис', 'кекс', 'кейс', 'кожа', 'коса', 'краб', 'крем', 'липа', 'лира', 'мама', 'мина', 'мною', 'море', 'мазь', 'нуга', 'ночь', 'нота', 'мочь', 'обед', 'орех', 'отец', 'пакт', 'окно', 'пара', 'папа', 'пика', 'пуля', 'ритм', 'риск', 'роза'],5: ['аббат', 'алтын', 'аршин', 'злато', 'байка', 'метал', 'милый', 'майка', 'иврит', 'игрек', 'жменя', 'балет', 'бедро', 'невод', 'нежно', 'нимфа', 'ножка', 'обруч', 'оксид', 'октет', 'омлет', 'кефир', 'кисть', 'взмах', 'венгр', 'вылет', 'глаза', 'горох', 'грамм', 'конус', 'короб', 'пакет', 'отвар'],6: ['август', 'аспект', 'арахис', 'гепард', 'голень', 'каштан', 'карась', 'климат', 'дракон', 'банкет', 'костёр', 'еловый', 'блесна', 'жаргон', 'жилище', 'курага', 'кунжут', 'ластик', 'волчок', 'льгота', 'манжет', 'икание', 'тесьма', 'примат', 'монета', 'мачете', 'натрий', 'наобум'],7: ['адвокат', 'акведук', 'вельвет', 'весёлый', 'вкратце', 'антоним', 'водород', 'жёсткий', 'журавль', 'забавно', 'вязание', 'запятая', 'записка', 'галоген'],8: ['авторейс', 'вращение', 'ансамбль', 'гардероб', 'гидроним', 'ипподром', 'испарина', 'запястье', 'досветла', 'комильфо', 'овертайм', 'либретто'],9: ['аккордеон', 'водоросль', 'багроветь', 'ветеринар', 'бизнесмен', 'видеоклуб', 'гастроном', 'депиляция', 'известняк', 'индикатор', 'зарубежье', 'еврокубок', 'житейский'],10: ['агрофизика', 'адаптивный', 'античность', 'аутсорсинг', 'благодушно', 'герцогство', 'главенство', 'закруглять', 'муравейник'],11: ['автоперевод', 'внедорожник', 'бестелесный', 'естествовед', 'долгосрочно', 'дезинфляция', 'импликатура', 'кислородный', 'мануфактура'],12: ['аккордеонист', 'безымянность', 'доминировать', 'католический', 'красноглазый']};
});
...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.words', 'game.entities.particle').defines(function() {
    EntityEnemy = ig.Entity.extend({word: 'none',remainingWord: 'none',health: 8,currentLetter: 0,targeted: false,font: new ig.Font('media/fonts/tungsten-18.png'),fontActive: new ig.Font('media/fonts/tungsten-18-orange.png'),speed: 10,friction: {x: 100,y: 100},hitTimer: null,dead: false,angle: 0,wordLength: {min: 8,max: 8},soundHit: new ig.Sound('media/sounds/hit.ogg'),type: ig.Entity.TYPE.B,checkAgainst: ig.Entity.TYPE.A,init: function(x, y, settings) {
        this.parent(x, y, settings);
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'plugins.impact-splash-loader').defines(function() {
    ...
    ZType = ig.Game.extend({font: new ig.Font('media/fonts/tungsten-18.png'),fontScore: new ig.Font('media/fonts/04b03-mono-digits.png'),fontTitle: new ig.Font('media/fonts/tungsten-48.png'),fontSelected: new ig.Font('media/fonts/tungsten-18-orange.png'),spawnTimer: null,targets: {},currentTarget: null,yScroll: 0,backdrop: new ig.Image('media/background/backdrop.png'),grid: new ig.Image('media/background/grid.png'),music: new ig.Sound('media/music/endure.ogg', false),menu: null,mode: 0,score: 0,streak: 0,hits: 0,misses: 0,multiplier: 1,wave: {},gameTime: 0,kills: 0,difficulty: 'NORMAL',init: function() {
        ...
        var first = 'а'.charCodeAt(0), last = 'я'.charCodeAt(0);
